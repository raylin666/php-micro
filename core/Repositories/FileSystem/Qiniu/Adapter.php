...
class Adapter implements FilesystemAdapter
{
    ...
    public function write(string $path, string $contents, Config $config): void
    {
        ...
        /**
         * @var null|Error $error
         */
        $returnBody = '{"bucket":$(bucket),"key":$(key),"name":$(fprefix),"size":$(fsize),"mimeType":$(mimeType),"hash":$(etag),"uuid":$(uuid),"width":$(imageInfo.width),"height":$(imageInfo.height),"ext":$(ext)}';
        $policy = [
            'returnBody' => $returnBody,
        ];

        [$response, $error] = $this->getUploadManager()->put(
            $this->getAuthManager()->uploadToken($this->bucket, null, 3600, $policy),
            $path,
            $contents,
            null,
            $mime,
...
var_dump($response);
        ...
        Context::set('upload.qiniu.response', $response);
    }
    ...
    protected function normalizeFileInfo(array $stats): FileAttributes
    {
        return new FileAttributes(
            ...
            $stats['fsize'] ?? null,
            null,
            isset($stats['putTime']) ? intval(floor($stats['putTime'] / 10000000)) : null,
            $stats['mimeType'] ?? null
        );
    }
